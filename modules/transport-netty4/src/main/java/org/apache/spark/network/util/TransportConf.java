...
public class TransportConf {
    ...
    public int connectionTimeoutMs() {
        ...
        long defaultTimeoutMs = JavaUtils.timeStringAsSec(conf.get(SPARK_NETWORK_IO_CONNECTIONTIMEOUT_KEY, defaultNetworkTimeoutS + "s"))
            * 1000;
    ...
    public int authRTTimeoutMs() {
        return (int) JavaUtils.timeStringAsSec(conf.get("spark.network.auth.rpcTimeout", conf.get(SPARK_NETWORK_SASL_TIMEOUT_KEY, "30s")))
            * 1000;
    }
    ...
    public int memoryMapBytes() {
        return JavaUtils.checkedCast(JavaUtils.byteStringAsBytes(conf.get("spark.storage.memoryMapThreshold", "2m")));
    }
    ...
    public int maxSaslEncryptedBlockSize() {
        return JavaUtils.checkedCast(JavaUtils.byteStringAsBytes(conf.get("spark.network.sasl.maxEncryptedBlockSize", "64k")));
    }
    ...
    */
    ...
    public int minChunkSizeInMergedShuffleFile() {
        return JavaUtils.checkedCast(
            JavaUtils.byteStringAsBytes(conf.get("spark.shuffle.push.server.minChunkSizeInMergedShuffleFile", "2m"))
        );
    }
