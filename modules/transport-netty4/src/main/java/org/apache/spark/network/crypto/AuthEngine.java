...
class AuthEngine implements Closeable {
    ...
    private AuthMessage encryptEphemeralPublicKey(byte[] ephemeralX25519PublicKey, byte[] transcript) throws GeneralSecurityException {
        ...
        byte[] aadState = JavaUtils.concat(appId.getBytes(UTF_8), nonSecretSalt, transcript);
        // Use HKDF to derive an AES_GCM key from the pre-shared key, non-secret salt, and AAD state
        byte[] derivedKeyEncryptingKey = Hkdf.computeHkdf(MAC_ALGORITHM, preSharedSecret, nonSecretSalt, aadState, AES_GCM_KEY_SIZE_BYTES);
    ...
    private byte[] decryptEphemeralPublicKey(AuthMessage encryptedPublicKey, byte[] transcript) throws GeneralSecurityException {
        assert (appId.equals(encryptedPublicKey.appId));
        // Mix in the app ID, salt, and transcript into HKDF and use it as AES-GCM AAD
        byte[] aadState = JavaUtils.concat(appId.getBytes(UTF_8), encryptedPublicKey.salt, transcript);
        // Use HKDF to derive an AES_GCM key from the pre-shared key, non-secret salt, and AAD state
    ...
    AuthMessage response(AuthMessage encryptedClientPublicKey) throws GeneralSecurityException {
        assert (appId.equals(encryptedClientPublicKey.appId));
    ...
    void deriveSessionCipher(AuthMessage encryptedClientPublicKey, AuthMessage encryptedServerPublicKey) throws GeneralSecurityException {
        assert (appId.equals(encryptedClientPublicKey.appId));
        assert (appId.equals(encryptedServerPublicKey.appId));
