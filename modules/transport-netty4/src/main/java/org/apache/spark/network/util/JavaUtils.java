...
public class JavaUtils {
    ...
    public static void deleteRecursively(File file, FilenameFilter filter) throws IOException {
        ...
        if (SystemUtils.IS_OS_UNIX && filter == null) {
            try {
                ...
                logger.warn(
                    "Attempt to delete using native Unix OS command failed for path = {}. " + "Falling back to Java IO way",
    ...
    private static final ImmutableOpenMap<String, TimeUnit> timeSuffixes = ImmutableOpenMap.<String, TimeUnit>builder()
        .fPut("us", TimeUnit.MICROSECONDS)
        .fPut("ms", TimeUnit.MILLISECONDS)
        .fPut("s", TimeUnit.SECONDS)
        .fPut("m", TimeUnit.MINUTES)
        .fPut("min", TimeUnit.MINUTES)
        .fPut("h", TimeUnit.HOURS)
        .fPut("d", TimeUnit.DAYS)
        .build();

    private static final ImmutableOpenMap<String, ByteUnit> byteSuffixes = ImmutableOpenMap.<String, ByteUnit>builder()
        .fPut("b", ByteUnit.BYTE)
        .fPut("k", ByteUnit.KiB)
        .fPut("kb", ByteUnit.KiB)
        .fPut("m", ByteUnit.MiB)
        .fPut("mb", ByteUnit.MiB)
        .fPut("g", ByteUnit.GiB)
        .fPut("gb", ByteUnit.GiB)
        .fPut("t", ByteUnit.TiB)
        .fPut("tb", ByteUnit.TiB)
        .fPut("p", ByteUnit.PiB)
        .fPut("pb", ByteUnit.PiB)
        .build();
    ...
    public static int checkedCast(long value) {
        ...
        checkArgument((long) result == value, "Out of range: %s", new Object[] { value });
        return result;
    }
