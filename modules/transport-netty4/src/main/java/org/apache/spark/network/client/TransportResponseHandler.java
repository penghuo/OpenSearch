...
public class TransportResponseHandler extends MessageHandler<ResponseMessage> {
    private static final Logger logger = LogManager.getLogger(TransportResponseHandler.class);
    ...
    public void channelInactive() {
        if (numOutstandingRequests() > 0) {
            ...
            logger.error("Still have {} requests outstanding when connection from {} is closed", numOutstandingRequests(), remoteAddress);
        ...
        if (message instanceof ChunkFetchSuccess) {
            ...
            if (listener == null) {
                ...
                listener.onFailure(
                    ...
                    new ChunkFetchFailureException("Failure while fetching " + resp.streamChunkId + ": " + resp.errorString)
                );
            }
            ...
            try {
                MergedBlockMetaResponseCallback listener = (MergedBlockMetaResponseCallback) outstandingRpcs.get(resp.requestId);
                if (listener == null) {
                    logger.warn(
                        "Ignoring response for MergedBlockMetaRequest {} from {} ({} bytes) since it is not" + " outstanding",
                        resp.requestId,
                        getRemoteAddress(channel),
                        resp.body().size()
                    );
