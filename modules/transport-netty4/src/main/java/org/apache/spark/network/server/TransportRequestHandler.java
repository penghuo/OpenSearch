...
public class TransportRequestHandler extends MessageHandler<RequestMessage> {
    ...
    private static final Logger logger = LogManager.getLogger(TransportRequestHandler.class);
    ...
    private void processStreamRequest(final StreamRequest req) {
        ...
        try {
            ...
            respond(new StreamFailure(req.streamId, JavaUtils.getStackTraceAsString(e)));
            return;
        }
        ...
        try {
            rpcHandler.receive(reverseClient, req.body().nioByteBuffer(), new RpcResponseCallback() {
                ...
                public void onFailure(Throwable e) {
                    respond(new RpcFailure(req.requestId, JavaUtils.getStackTraceAsString(e)));
                }
            });
        } catch (Exception e) {
    ...
    private void processStreamUpload(final UploadStream req) {
        ...
        try {
                ...
                public void onComplete(String streamId) throws IOException {
                    try {
                        ...
                        IOException ioExc = new IOException(
                            "Failure post-processing complete stream;" + " failing this rpc and leaving channel active",
                            ex
                        );
            ...
            if (e instanceof BlockPushNonFatalFailure) {
                ...
                respond(new RpcResponse(req.requestId, new NioManagedBuffer(((BlockPushNonFatalFailure) e).getResponse())));
            } else {
                logger.error("Error while invoking RpcHandler#receive() on RPC id " + req.requestId, e);
                respond(new RpcFailure(req.requestId, JavaUtils.getStackTraceAsString(e)));
            }
    ...
    private void processMergedBlockMetaRequest(final MergedBlockMetaRequest req) {
        try {
            rpcHandler.getMergedBlockMetaReqHandler().receiveMergeBlockMetaReq(reverseClient, req, new MergedBlockMetaResponseCallback() {
            ...
            logger.error(
                String.format(
                    "Error while invoking receiveMergeBlockMetaReq() for appId %s shuffleId %s reduceId %s",
                    req.appId,
                    req.shuffleId,
                    req.appId
                ),
