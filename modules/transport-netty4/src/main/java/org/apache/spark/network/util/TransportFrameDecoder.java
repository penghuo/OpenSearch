...
public class TransportFrameDecoder extends ChannelInboundHandlerAdapter {
    ...
    private ByteBuf decodeNext() {
        ...
        if (frameBuf == null) {
            JavaUtils.checkArgument(frameSize < MAX_FRAME_SIZE, "Too large frame: %s", frameSize);
            JavaUtils.checkArgument(frameSize > 0, "Frame length should be positive: %s", frameSize);
    ...
    public void setInterceptor(Interceptor interceptor) {
        assert this.interceptor == null : ("Already have an interceptor.");
