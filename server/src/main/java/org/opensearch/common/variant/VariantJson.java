 ...
 * easier implementation would read the subtree into a {@code Map} and encode that, at the cost of walking every
 * document twice.
 *
 * @opensearch.internal
 */
public final class VariantJson {
     ...
     *
     * <p>The parser is the only source of type information, so the encoding records what it saw and no more. JSON text
     * gives it no way to tell {@code 200} meant as a {@code long} from {@code 200} meant as an {@code int}, and nothing
     * downstream can recover that distinction either.
     */
