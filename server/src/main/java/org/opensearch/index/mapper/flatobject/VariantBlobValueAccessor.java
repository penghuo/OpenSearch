...
/**
 * Solution B: reads the field's value out of a Variant blob held in a {@code BinaryDocValues} column.
 *
...
public class VariantBlobValueAccessor implements FlatObjectValueAccessor {
    ...
    public void setNextReader(LeafReaderContext context) throws IOException {
        ...
        // Two superseded prototype layouts also put a column under this name -- deduplicated key sets, and a per-document
        // rank list. Neither is written any more and neither is read. Refuse such a segment rather than read it as though its
        // field ids were already in name order, which would return values under the wrong keys without erring.
     ...
     *
     * <p>Scalars are converted to boxed Java values here rather than in the coercion layer, so both arms hand the shared
     * coercion table the same types and a divergence can only come from what was stored. Containers are only materialised
     * when the caller actually asked for the raw value.
     */
    ...
    /**
     * Navigator over a Variant, mirroring {@link PathResolver#MAP_NAVIGATOR} so both arms resolve paths identically.
     *
    ...
    private Variant variant(int docId) throws IOException {
        ...
        // BinaryDocValues is a forward-only iterator, but the fetch phase and the point-read benchmark both address
        // documents out of order. Without this restart a backwards read would silently return no value, which would look
        // like a correctness bug in one place and like a suspiciously fast result in the other.
