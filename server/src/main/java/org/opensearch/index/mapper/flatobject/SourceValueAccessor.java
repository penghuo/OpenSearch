...
/**
 * Solution A: reads the field's value out of {@code _source}.
 *
 * <p>Per access this decompresses the stored-fields block holding the document and parses the <b>whole</b> document's
 * JSON in order to reach one path. That cost is the design's central claim about this arm, so it is important that the
 * implementation neither inflates nor understates it:
 *
 * <ul>
 *   <li>The parsed document is cached for the <b>current document only</b>, mirroring how {@code SourceLookup} behaves
 *       during a real search. Without the cache, reading three paths from one document would parse it three times and
 *       Solution A would look worse than it is in production.
 *   <li>The cache is deliberately not larger than one document. A larger cache would make a benchmark that revisits
 *       documents measure the cache rather than the store.
 * </ul>
 *
 * @opensearch.internal
 */
