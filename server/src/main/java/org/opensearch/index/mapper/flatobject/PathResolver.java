 ...
 * blob, and the two must name the same value. Held separately, the rule below would drift, and a query would answer
 * differently depending on which store served it. So the rule lives here once and each store supplies only a
 * {@link Navigator}.
 *
