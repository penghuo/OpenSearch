 ...
 *
 * <p>This is the controlled-variable boundary of the whole comparison. Two implementations exist — one backed by
 * {@code _source}, one by a Variant blob in a {@code BinaryDocValues} column — and everything above this interface is
 * shared. Any difference in latency, allocation or result between the two is therefore a property of the value store.
 *
...
public interface FlatObjectValueAccessor {
     ...
     *
     * <p>Test C1.6 requires both arms to exclude exactly the same values, so this count is part of the comparison rather
     * than a diagnostic.
     */
     ...
     *
     * <p>False for the {@code _source} implementation on an index with {@code _source} disabled, which is the condition
     * test C3.1 turns into a result.
     */
    boolean valueStoreAvailable();

    /**
     * A short stable name for the backing store, used to label benchmark and correctness output.
     */
    String storeName();
}
