...
public class OtelDocGeneratorTests extends OpenSearchTestCase {
    ...
    /**
     * The point-read benchmark addresses documents out of order, so random access must agree with the sequential stream.
     */
     ...
     * the reader, not the document. That is exactly the mechanism by which Solution A can lose integer width while
     * Solution B, which records a type tag at write time, does not. This test is deliberately an assertion about
     * current behaviour: if it ever starts failing, the A-versus-B fidelity comparison needs revisiting.
     */
