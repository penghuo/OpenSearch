...
/**
 * Generates a deterministic synthetic OTel log corpus for the {@code flat_object} value-store comparison.
 *
 ...
 *       so document <i>i</i> can be produced without replaying the stream. The point-read benchmark reads documents in a
 *       pseudo-random order and needs to know what it should have got back.
 *   <li><b>Byte-identical reproduction.</b> The same {@link CorpusConfig} always yields the same bytes, so Solution A and
 *       Solution B are compared over provably identical input. Any measured difference is then attributable to the value
 *       store rather than to corpus drift.
 * </ol>
 *
...
public final class OtelDocGenerator implements Iterable<byte[]> {
    ...
    /** Keys present in every document. Benchmarks read these paths; C1.4 groups by {@link #KEY_K8S_NAMESPACE}. */
    ...
    /** The stable value type of a key, exposed so a benchmark can build a matching derived field. */
     ...
     * the key name, so the benchmark asks for one rather than assuming.
     */
     ...
     * attributes-size target and to predict Solution B's blob column size offline.
     */
