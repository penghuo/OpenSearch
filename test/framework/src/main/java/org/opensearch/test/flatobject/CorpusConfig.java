 ...
 *
 * <p>The knobs mirror test-plan section 2.2: document size, the size of the {@code attributes} value, keys per document,
 * type mix, and nesting depth.
 *
 ...
 * document size can be swept independently. The latter is what makes the read-granularity experiment decidable: only by
 * holding attributes fixed while growing the document can "B tracks attributes bytes, A tracks whole-document bytes" be
 * distinguished from "both track document bytes".
 */
public final class CorpusConfig {

    /**
     * The kinds of value that may appear at an attribute leaf. Covers every case enumerated in correctness test C1.1.
     */
     ...
     * supposed to win on. With filler removed, {@code attributes} is most of the document and the comparison runs on the
    ...
    public static CorpusConfig preset(String presetName) {
        switch (presetName) {
            ...
            // Smallest end-to-end scale: ~1.1GB raw. Indexes and force-merges in a couple of minutes per arm, so the
            ...
            // baseline verdict is in. Expect roughly an hour to index and longer to force merge, per arm.
            case "S1":
            ...
            // Expectation under the design's hypothesis: B flat, A rising.
            case "S2_SIZE_512":
            ...
            // Expectation under the design's hypothesis: B rising, A flat-ish.
            case "S2_ATTR_288":
     ...
     * index and hours of indexing per arm.
     */
    ...
    /** Every preset that a benchmark run indexes. */
